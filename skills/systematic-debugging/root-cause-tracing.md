...
```text
Error: git init failed in C:\Users\skyler\project\packages\core
```
...
**运行并捕获：**
在 Windows Trae 主机上优先使用 PowerShell 兼容写法：

```powershell
npm test 2>&1 | Select-String 'DEBUG git init'
```

如果命令明确运行在容器、远程 Linux 或 WSL 中，再改用对应环境的 POSIX 管道。
...
这个脚本本身是 POSIX shell 脚本，更适合在 Git Bash、WSL 或 Linux/macOS 环境中运行。Windows Trae 主机如果没有兼容 shell，应把它视为参考思路，改用 PowerShell 或测试框架原生命令逐步缩小范围。逐个运行测试，在第一个“污染者”处停止。详见脚本中的使用说明。
