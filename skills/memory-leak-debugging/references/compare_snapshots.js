...
// This fallback helper is a human-run investigative script. Console-oriented
// output is intentional because it is used to inspect heap snapshot growth
// interactively when memlab is unavailable.
...
if (!file1 || !file2) {
  ...
  console.error(
    'Windows PowerShell example: node "C:\\Users\\skyler\\.trae\\skills\\memory-leak-debugging\\references\\compare_snapshots.js" "C:\\path\\baseline.heapsnapshot" "C:\\path\\target.heapsnapshot"',
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    'Error parsing snapshots. They may be invalid, too large for JSON.parse, or not Chrome/Node heap snapshot JSON.',
  );
