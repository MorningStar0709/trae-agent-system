...
description: Use when the user wants to create, name, split, review, or optimize a concrete Trae agent configuration, including English identifier, Chinese display name, trigger description, system prompt, capability toggles, MCP binding strategy, or agent boundary design. 仅当用户明确要新建、拆分、命名或优化一个具体 agent 时使用，适用于“帮我创建一个 agent”“这个 agent 该怎么命名”“给我 agent 提示词”“什么时候触发这个 agent”“拆一个专门做 X 的 agent”“这个 MCP 适合独立 agent 吗”等请求。Do not use for Trae Skill creation or SKILL.md review, general prompt writing, software architecture, workflow planning, ordinary coding, or general agent concept discussion unless the user is preparing a concrete reusable Trae agent configuration.
---
